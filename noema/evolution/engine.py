...
"""
    ...
    async def apply_patch(self, patch: EvolutionPatch, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            try:
                ...
                path.write_text(patch.original_code, encoding="utf-8")
                patch.applied = False
    ...
    async def run_evolution_cycle(self, test_runner: Callable | None = None) -> EvolutionResult:
        ...
        for issue in analysis.get("issues", [])[: self.max_patches_per_cycle]:
            patch = await self.generate_patch(issue["file"], issue)
            if patch:
                self.patches.append(patch)
                result.patches_generated += 1

                success = await self.apply_patch(patch, dry_run=True)
                if success:
                    if test_runner:
                        try:
                            test_ok = await test_runner()
                            patch.tests_passed = test_ok
                        except Exception:
                            patch.tests_passed = False
                    else:
                        patch.tests_passed = True

                    if patch.tests_passed and patch.confidence >= 0.7:
                        applied = await self.apply_patch(patch, dry_run=False)
                        if applied:
                            result.patches_applied += 1
                            result.improvements.append(
                                f"Applied patch to {patch.target}: {patch.description}"
                            )
                        else:
        ...
        result.summary = (
            ...
            f"{result.patches_applied} applied, {result.patches_rejected} rejected"
        )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "applied": applied,
            "rejected": total_patches - applied,
